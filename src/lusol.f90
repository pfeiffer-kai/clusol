...
! Contains    lu6sol   lu6L     lu6Lsparse lu6Lt    lu6U     Lu6Ut    lu6LD    lu6chk
! Contains    lu7add   lu7cyc   lu7elm   lu7for   lu7rnk   lu7zap
! Contains    lu8rpc lu8rpcsparse
!
! Contains    jdamax
!
...
module lusol
    ...
    implicit none
    private
    public    :: lu1fac, lu6sol, lu8rpc, lu8rpcsparse
    private   :: jdamax
...
contains
    ...
    subroutine lu1fac( m    , n    , nelem, lena , luparm, parmlu,       &
            a    , indc , indr , p    , q     ,               &
            lenc , lenr , locc , locr ,                       &
            iploc, iqloc, ipinv, iqinv, w     , inform, n1,n2,np )

        integer(ip),   intent(in)    :: m, n, nelem, lena,n1,n2,np
        ...
        logical                :: keepLU, TCP, TPP, TRP, TSP, TCPNM
        ...
        ! print *,"lu1fac n1",n1,"n2",n2,"np",np
        ...
        TCPNM  = lPiv == 4  ! Threshold Symmetric Pivoting.
        kPiv(0)= 'PP'
        kPiv(1)= 'RP'
        kPiv(2)= 'CP'
        kPiv(3)= 'SP'
        ...
        ! PRINT *,"===clu1fac::parmlu(4) ",parmlu(4)
        ! write(msg,'(F20.10)')parmlu(4)
        ! CALL PERROR(msg)
        ...
        call lu1fad( m     , n     , numnz , lena2 , luparm, parmlu,       &
            ...
            Lmax  , Umax  , DUmax , DUmin , Akmax, n1, n2, np )
        ...
        if ( TPP ) then
            growth = Ugrwth
        else ! TRP or TCP or TSP or TCPNM
            growth = Agrwth
        end if
        parmlu(16) = growth
    ...
    subroutine lu1fad( m     , n     , nelem , lena  , luparm, parmlu,   &
        ...
        integer(ip),   intent(in)    :: m, n, nelem, lena, lenH, nslack, n1, n2, np
        integer(ip),   intent(inout) :: luparm(30)
        ...
        logical                :: Utri, Ltri, spars1, spars2, dense,       &
            densLU, keepLU, TCP, TPP, TRP, TSP, TCPNM
        ...
        integer(ip)       :: Hlen, Hlenin, hops, h,                &
            ...
            mark, maxcol, maxmn, maxrow, mbest, mbestnew,   &
            melim, minfre, minmn, mleft,          &
            mrank, ncold, nelim, nfill,           &
            nfree, nleft, nout, nrowd, nrowu,     &
            nsing, nspare, nzchng, nzleft, npctr, npctrLim, nz
        ...
        TCP    = lPiv == 2  ! Threshold Complete  Pivoting.
        TSP    = lPiv == 3  ! Threshold Symmetric Pivoting.
        TCPNM  = lPiv == 4  ! Threshold Complete  Pivoting without Markovitz
        ...
        npctr = 0
        ! print*,"lu1fad compute npctrlim with",n2-n1+1,np
        ! npctrLim = n2 - n1 + 1 - np
        ! npctrLim = 0
        npctrLim = 1e10
        ...
        if (TCP .or. TCPNM) then ! Set Ha(1:Hlen) = biggest element in each column,
        ...
        !     do lq = lq1, lq2
        !     j      = q(lq)
        !     print*,"lu1fad nz",nz,"j",j,"lq1",lq1,"lq2",lq2,"at nrowu",nrowu
        !     end do
        ! end if
        ! end do
        ...
        if ( TCP .or. TCPNM ) then
            if (n1 < 0) then
                aijmax = Ha(1)      ! Marvelously easy !
            else  
                ! find the strongest column
                ! only consider the n2-n1-np strongest columns from the range
                do h = 1, Hlen
                j = Hj(h)
                ! print*,"Ha(",h,")",Ha(h),"j",j,"npctr < npctrLim",npctr,npctrLim 
                if (n1 <= j .and. j <= n2) then
                    if (npctr + 1 <= npctrLim) then
                        aijmax = Ha(h)
                        exit
                    else
                        cycle
                ...
                aijmax = Ha(h)      ! Marvelously easy !
                ! print*,"lu1fad search for aijtol, j",j,"n1 n2",n1,n2,"aijtol",aijmax,"Hlen",Hlen
                exit
                end do
            end if
            ...
            aijtol = aijmax / Ltol
            if ( TCPNM ) then
                ! Kai: use the largest matrix element as pivot
                jbest = Hj(1)
                lc     = locc(jbest)
                ibest  = indc(lc)
                abest  = a(lc)
                kbest = lenc(jbest)
                mbest = lenr(ibest) * lenc(jbest) ! merit
                ! break ties
                do h = 2, Hlen
                j  = Hj(h)
                lc = locc(j)
                i  = indc(lc) ! gives 0 sometimes? where's the bug?
                ! i      = indc(lc)
                ! len1   = lenr(i) - 1
                if (abs(abest) == abs(a(lc))) then
                    if (i > 0) then 
                        mbestnew = lenr(i) * lenc(j)
                        ! print *,"h",h,"i",i,"j",j,"jbest",jbest,"maxaj",a(lc),"lenr",lenr(i), &
                        !     "lenc",lenc(j),"kbest",kbest,"merit",mbest,"meritnew",mbestnew
                    else
                        mbestnew = 1e7
                        ! print *,"h",h,"i",i,"j",j,"jbest",jbest,"maxaj",a(lc), &
                        !     "lenc",lenc(j),"kbest",kbest,"merit",mbest,"meritnew",mbestnew
                    end if
                    ! leftest column
                    ! if (lenc(j) > 0 .and. j < jbest) then
                    !     jbest  = j
                    !     ibest  = indc(lc)
                    !     kbest = lenc(j)
                    ! end if
                    ! column length based, shortest and leftest column
                    ! weird behaviour, lenc (and especially lenr) become 0 / negative at times?!
                    if (lenc(j) > 0) then
                        if (lenc(j) < kbest) then
                            jbest  = j
                            ibest  = indc(lc)
                            kbest = lenc(j)
                        else if (lenc(j) == kbest .and. j < jbest) then
                            jbest  = j
                            ibest  = indc(lc)
                            kbest = lenc(j)
                        endif
                        ! else keep first find
                    endif
                    ! ! merit based ! this seems to be slower than column length based search
                    ! if (i == 0) then
                    !     if (lenc(j) > 0) then
                    !         if (lenc(j) < kbest) then
                    !             jbest  = j
                    ...
                    ! else
                    !     if (lenc(j) > 0) then
                    !         if (mbestnew < mbest) then
                    !             jbest  = j
                    !             ibest  = indc(lc)
                    !             kbest = lenc(j)
                    !             mbest = mbestnew
                    !         else if (mbestnew == mbest .and. j < jbest) then
                    !             jbest  = j
                    !             ibest  = indc(lc)
                    !             kbest = lenc(j)
                    !             mbest = mbestnew
        ...
        if ( .not. TCPNM ) then
            ...
            if ( Utri ) then
                !------------------------------------------------------------
                ! So far all (kai: previoulsy chosen pivot) columns have had length 1 (kai: these columns are referred to as slack).
                ...
                if (kslack < nslack) then
                    ...
                    if (w(j) > zero) then ! Accept a slack
                        if (n1 <= j .and. j <= n2) then
                            if (npctr + 1 > npctrLim) then
                                ! print*,"lu1fad: the column",j,"is in the range",n1,"-",n2,"with npctr npctrlim",npctr,npctrLim
                                cycle
                            else
                                ! print*,"lu1fad: the column",j,"is in the range",n1,"-",n2," BUT CAN BE USED &
                                !     with npctr npctrlim",npctr,npctrLim
                                npctr = npctr + 1
                            end if
                        end if
                        kslack = kslack + 1
                        ...
                        abest  = a(lc)
                        mbest  = 0
                        ! print*,"goto 300 1 with npctr npctrLim",npctr,npctrLim
                        go to 300
                    end if
                    end do
                ...
                if (lq1 <= lq2) then  ! There are more cols of length 1.
                    ...
                    250           if (jbest > 0) then
                        ...
                        ibest  = indc(lc)
                        mbest  = 0
                        ! print*,"goto 300 2 with npctr npctrLim",npctr,npctrLim
                        go to 300
                    end if
                end if
            ...
            if ( spars1 ) then
                ...
                if (TPP .or. TCP) then
                    call lu1mar( m    , n     , lena  , maxmn,          &
                        ...
                        a    , indc  , indr  , p     , q,      &
                        lenc , lenr  , locc  , locr  ,         &
                        iploc, iqloc, n1,n2,np,npctr,npctrLim )
        ...
        ! print *,"lu1fad ibest",ibest,"jbest",jbest,"abest",abest, &
        !     "kbest",kbest,"mbest",mbest,"TCP",TCP,"TCPNM",TCPNM,"dense",dense,"denseLU",densLU
        nrowd  = lenc(jbest)
        melim  = nrowd  - 1
        nelim  = ncold  - 1
        mersum = mersum + mbest
        ...
        if (Utri .and. TPP) then
            ...
            if (nelim > 0) then
                ...
                if (TCP .or. TCPNM) then ! Update modified columns in heap
        ...
        if ( densLU ) then
            ...
            call lu1ful( m     , n    , lena , lenD , lu1 , TPP, &
                ...
                lenc  , lenr , locc , ipinv, locr, n1,n2,np,npctr, npctrLim )
    ...
    subroutine lu1mar( m    , n     , lena  , maxmn,          &
            ...
            ibest, jbest , mbest ,                 &
            a    , indc  , indr  , p     , q    ,  &
            lenc , lenr  , locc  , locr  , iploc, iqloc, n1,n2,np,npctr,npctrLim )

        logical,       intent(in)    :: TCP
        integer(ip),   intent(in)    :: m, n, lena, maxmn, maxcol, maxrow, n1,n2,np
        real(rp),      intent(in)    :: aijtol, Ltol, a(lena)
        integer(ip),   intent(in)    :: indc(lena), indr(lena), p(m)    , q(n)    , &
            lenc(n)   , lenr(m)   , iploc(n), iqloc(m), &
            locc(n)   , locr(m)
        integer(ip),   intent(out)   :: ibest, jbest, mbest, npctr, npctrLim
        ...
        !------------------------------------------------------------------
        ! print*,"lu1mar: range",n1,"-",n2,"with np, npctr, npctrLim",np,npctr,npctrLim
        ...
        !     do lq = lq1, lq2
        !     j      = q(lq)
        !     print*,"0 = lu1mar nz",nz,"j",j,"lq1",lq1,"lq2",lq2
        !     end do
        ! end if
        ! end do
        ...
        if (nz    >  m  ) go to 200
        ! print*,"1 === lu1mar nz",nz,"kbest",kbest,"nz1",nz1,"m",m
        ...
        lc2    = lc1 + nz1
        amax   = abs( a(lc1) )
        ! print*,"2 ===== lu1mar nz",nz,"j",j,"amax",amax,"Cols lq1 lq2",lq1,lq2,"lc1 lc2",lc1,lc2
        ...
        if ( TCP ) then
            if (amax < aijtol) then
                ! print*,"2 Cols amax tol",amax,aijtol
                cycle Cols ! Nothing in whole column
            end if
        end if
        ...
        i      = indc(lc)
        len1   = lenr(i) - 1
        ! print*,"3 ===== lu1mar lc i len1",lc,i,len1
        ! merit  = nz1 * len1
        ! if (merit > mbest) cycle
        if (len1  > kbest) then
            ! print*,"3 ===== lu1mar len1 > kbest",len1,kbest
            cycle Colj
        end if
        ...
        if (lc == lc1) then
            ...
            if ( TCP ) then ! Absolute test for Complete Pivoting
                if (aij      < aijtol) then
                    ! print*,"3 Colj amax tol",amax,aijtol
                    cycle Colj
                end if
            else !!! TPP
        ...
        ! print*,"4 ===== lu1mar lc i len1",lc,i,len1
        ...
        if (merit == mbest) then
            ...
            if (lbest <= gamma  .and.  cmax <= gamma) then
                ...
                if (lbest <= cmax) then
                    ! print*,"3 Colj lbest cmax",lbest,cmax
                    cycle Colj
                end if
            end if
        end if
        ! print*,"5 ===== lu1mar lc i len1",lc,i,len1
        ...
        if (ibest == 0) then
            ! print*,"lu1mar: error, ibest has not been assigned, set npctrLim to 1e10"
            npctrLim = 1e10
            goto 9000
        end if
    ...
    subroutine lu1ful( m     , n    , lena , lenD , lu1 , TPP, &
            ...
            keepLU, small,                          &
            a     , d    , indc , indr , p   , q,   &
            lenc  , lenr , locc , ipinv, ipvt, n1,n2,np,npctr,npctrLim )

        logical,       intent(in)    :: TPP, keepLU
        integer(ip),   intent(in)    :: m, n, lena, lenD, lu1,   &
            mleft, nleft, nrank, nrowu, n1,n2,np,npctrLim
        ...
        integer(ip),   intent(out)   :: ipvt(m), ipinv(m), npctr   ! workspace
        ...
        if ( TPP ) then
            ...
            call lu1DCP( d, mleft, mleft, nleft, small, nsing, ipvt, q(nrowu), n1,n2,np,npctr,npctrLim )
        end if
    ...
    subroutine lu1DCP( a, lda, m, n, small, nsing, ipvt, q, n1,n2,np,npctr,npctrLim )

        integer(ip),   intent(in)    :: lda, m, n, n1,n2,np,npctrLim
        real(rp),      intent(in)    :: small

        integer(ip),   intent(inout) :: q(n)
        real(rp),      intent(inout) :: a(lda,n)

        integer(ip),   intent(out)   :: nsing, npctr   ! not used outside
        integer(ip),   intent(out)   :: ipvt(m)
        ...
        if (aijmax < ajmax) then
            ! print*,"lu1DCP j q(j) npctr npctrLim",jmax,q(j),npctr,npctrLim
            if (n1 <= q(j) .and. q(j) <= n2) then
                if (npctr + 1 > npctrLim) then
                    ! print*,"lu1DCP: the column",q(j),"is in the range",n1,"-",n2,"with npctr npctrlim",npctr,npctrLim
                    cycle
                else
                    ! print*,"lu1DCP: the column",q(j),"is in the range",n1,"-",n2," BUT CAN BE USED &
                    !     with npctr npctrlim",npctr,npctrLim
                    npctr = npctr + 1
                end if
            end if
        ...
        ! print *,"lu1dcp out imax",imax,"jmax",jmax,"aijmax",aijmax
    ...
    subroutine lu6sol( mode, m, n, v, w,       &
        ...
        !  11   v  sparse solves   L v = v(input).   w  is not touched.
        !
    ...
    subroutine lu6solsparse( mode, m, n, v, w,       &
            lena, luparm, parmlu,   &
            a, indc, indr, p, q,    &
            lenc, lenr, locc, locr, &
            inform, lmin )

        integer(ip),   intent(in)    :: mode, m, n, lena, lmin
        integer(ip),   intent(in)    :: indc(lena), indr(lena), p(m), q(n),   &
            lenc(n), lenr(m), locc(n), locr(m)
        real(rp),      intent(in)    :: a(lena)
        ...
        if      (mode == 1) then             ! Solve  L v(new) = v.
            call lu6Lsparse  ( inform, m, n, v,    &
                lena, luparm, parmlu, a, indc, indr, lenc, lmin )
    ...
    subroutine lu6L  ( inform, m, n, v, &
        ...
        !print*,"lu6lsparse numL0",numL0,"lenL0",lenL0,"lenL",lenL
        !print*,"m",m
        !    do i = 1, m !this loop can be shortened by passing sparse v
        !        ! l is the row of v, so column of L
        ...
        ! print*, "j",indc(l),"a(", lena-l+1, "): ", a(l), "v(",ipiv,")", vpiv
        ...
        ! numL gets large if there are allot of updates
        ! v is usually sparse so it makes more sense to loop through it instead
        do ldummy = 1, numL
        l      = l - 1
        ! if (abs(a(l)) > small) print*, "a(", l, "): ", a(l)
        i      = indr(l)
        ! print*, "i",i,"j",indc(l),"ldummy+lenL0",ldummy+lenL0,"a(", l, "): ", a(l), "v(i)", v(i)
        if (abs(v(i)) > small) then
            j    = indc(l)
            v(j) = v(j) + a(l)*v(i)
        end if
    ...
    subroutine lu6Lsparse  ( inform, m, n, v, &
            lena, luparm, parmlu, a, indc, indr, lenc, lmin )

        integer(ip),   intent(in)    :: m, n, lena, lmin
        ...
        !------------------------------------------------------------------
        ! lu6Lsparse   sparse solves   L v = v(input).
        !
        ! 15 Dec 2002: First version derived from lu6sol.
        ! 15 Dec 2002: Current version.
        ! 13 Dec 2011: First f90 version.
        ...
        integer(ip) :: i, ipiv, j, k, l, l1, ldummy, len, lenL, lenL0, numL, numL0, lmin_
        real(rp)    :: small, vpiv
        ...
        ! print*,"lu6lsparse numL0",numL0,"lenL0",lenL0,"lenL",lenL,"lmin",lmin!,"numl",numL
        ! print*,"m",m
        !     do i = 1, m !this loop can be shortened by passing sparse v
        !         ! l is the row of v, so column of L
        !        if (abs(v(i)) > small) then
        ...
        ! print*,"numL",numL,"lmin_",lmin_,"lenL",lenL
        do ldummy = 1, numL
        l      = l - 1
        i      = indr(l)
        ! print*, "i",i,"j",indc(l),"ldummy+lenL0",ldummy+lmin_,"a(", l, "): ", a(l), "v(i)", v(i)
        if (abs(v(i)) > small) then
            j    = indc(l)
            v(j) = v(j) + a(l)*v(i)
        end if
    ...
    subroutine lu7rnk( m, n, jsing, lena, parmlu,             &
        ...
        800 inform = 1
        write(*,*) 'Fatal error in LUSOL lu7rnk. rank=0. Stopping now'
        ! stop !kai: commented this
    ...
    subroutine lu8rpc( mode1, mode2, m, n, jrep, v, w, &
        ...
        ! If  mode2 = 3,  the new column is considered singular and will not be permuted
        !
        ...
        ! inform =  9  fatal error in lu7rnk
        !
        ...
        ! PRINT *,"===lu8rpc::mode1",mode1,"mode2",mode2
        ! PRINT *,"===lu8rpc::mode1",mode1,"mode2",mode2,"Utol1",Utol1,"Utol2",Utol2
        ...
        ! kai
        ! otherwise access error in q(nrank)
        if (nrank == 0) go to 900 
        ...
        if (nrank < n) then
            ...
            ! print*,"krep",krep,"rank",nrank
            if (krep < nrank) then
                klast    = nrank
            else
                if (mode2 == 3) then
                    ! we free the column nrank+1
                    q(krep) = q(nrank+1)
                    ! we swap the new column into position nrank+1
                    q(nrank+1) = jrep
                    krep     = nrank+1
                    ! finish the update
                    go to 920
    ...
    ! lu8rpcsparse
    !
    ! pseudo-sparse LU update: Replace Column
    ! only consider rows of L that are below and including the first nnz of v
    ! LUSOL's sparse implementation of the Bartels-Golub update.
    !
    ! 01 May 2002: Derived from LUSOL's original lu8a.f file.
    ! 01 May 2002: Current version of lusol8a.f.
    ...
    subroutine lu8rpcsparse( mode1, mode2, m, n, jrep, v, w, &
            lena, luparm, parmlu,           &
            a, indc, indr, p, q,            &
            lenc, lenr, locc, locr,         &
            inform, diag, vnorm, lmin )

        integer(ip),   intent(in)    :: mode1, mode2, m, n, jrep, lena, lmin
        integer(ip),   intent(inout) :: luparm(30), &
            indc(lena), indr(lena), p(m), q(n), &
            lenc(n), lenr(m), locc(n), locr(m)
        real(rp),      intent(inout) :: parmlu(30), a(lena), v(m), &
        ...
        logical                :: singlr
        integer(ip)            :: iw, j1, jsing, klast, krep, &
            l1, lenL, lenU, lprint, lrow, nout, nrank, nrank0, kk, i
        real(rp)               :: Utol1, Utol2, small, vi
        ...
        ! PRINT *,"lu8rpcsparse::mode2",mode2,"mode1",mode1,"lmin",lmin
        ! PRINT *,"===lu8rpc::mode1",mode1,"mode2",mode2,"Utol1",Utol1,"Utol2",Utol2,"parmlu4",parmlu(4)
        ...
        if (mode2 == 0) then
            ...
            if (mode2 == 1 .or. mode2 == 3) then
                ...
                call lu6solsparse( i1, m, n, v, w, lena, luparm, parmlu, &
                    a, indc, indr, p, q,                  &
                    lenc, lenr, locc, locr, inform, lmin )
                ! call lu6sol( i1, m, n, v, w, lena, luparm, parmlu, &
                !     a, indc, indr, p, q,                  &
                !     lenc, lenr, locc, locr, inform )
            end if
            ...
            if (mode2 == 3) then
                ...
                if (vi > small) then
                    inform = 1
                    ! print *,"lu8rpcsparse: mode 3: there should be no off diagonal element"
                    exit
                end if
                end do
                if (inform /= 1) then
            ...
            ! print *,"klast",klast,"after v lu7add to col",jrep
            if (inform == 7) go to 970
        end if
        ...
        if (mode2 == 0) then
            ...
            if (inform == 1) then
                ...
                ! Increase nrank by 1 and make klast point to the bottom.
                if (mode2 == 3) then
                    write(msg,"(A)")'lusol.f90: lu7elm deteced rank increase even though mode2=3'
                    CALL PERROR(msg)
                end if
                nrank = nrank + 1
                klast = nrank
        ...
        if (nrank < n) then
            ...
            if (krep < nrank) then
                ...
                if (mode2 == 3) then
                    ...
                    print *,"lusol.f90: clu8rpcsparse: mode2=3: We should not be here!!!"
                    write(msg,"(A)")'lusol.f90: clu8rpcsparse: mode2=3: We should not be here!!!'
                    CALL PERROR(msg)
                    ! print *,"krep",krep,"q(krep)",q(krep),"jrep",jrep,"q(nrank+1)",q(nrank+1)
                    ! we swap the new column into position nrank+1
                    q(krep) = q(nrank+1) ! q(krep) = jrep before
                    q(nrank+1) = jrep
                    ! q(jrep) = nrank+1
                    ! q(nrank+1) = q(krep) ! q(krep) = jrep before
                    ! call lu7cyc( krep, nrank+1, q )
                    ! print *,"2:krep",krep,"q(krep)",q(krep),"jrep",jrep,"q(nrank+1)",q(nrank+1)
                    go to 920
                end if
            else
                if (inform /= 1 .and. mode2 == 3) then
                    ! we free the column nrank+1
                    q(krep) = q(nrank+1)
                    ! we swap the new column into position nrank+1
                    q(nrank+1) = jrep
                    ...
                    if (mode2 == 3) then
                        write(msg,"(A)")'lusol.f90: clu8rpcsparse: mode2 is 3 but inform is 1'
                        CALL PERROR(msg)
                    end if
                    q(krep ) = q(nrank)
                    q(nrank) = jrep
